...
def calculate_finger_angles(joint_angles, joint_xyz):
  ...
  joint_angles[12] = angle(joint_xyz[1], joint_xyz[2], joint_xyz[3]) * 1.3  # Amplify thumb movement
...
def transmit_angles_serial(ser,joint_angles):
  ...
  # Convert joint angles to integers and clip to valid range (0-255)
  joint_angles_int = np.clip(joint_angles, 0, 255).astype(int)
  
  # Generate checksum
  sum = int(np.sum(joint_angles_int))
  sum = sum & 0x000000FF
  t_xchecksum = 255 - sum
...
parser.add_argument('--serial-port', type=str, default='COM14', help='Set serial port (default=COM14)')
